...
type Address struct {
	Id        int    `orm:"column(addr_id);auto"`
	Uid       *User  `orm:"column(uid);rel(fk)"`
	AddrName  string `orm:"column(addr_name);size(10);null"`
	Fullname  string `orm:"column(fullname);size(8);null"`
	Phone     string `orm:"column(phone);size(20);null"`
	Province  string `orm:"column(province);size(10);null"`
	City      string `orm:"column(city);size(10);null"`
	Region    string `orm:"column(region);size(10);null"`
	Street    string `orm:"column(street);size(50);null"`
	IsDefault string `orm:"column(is_default);null"`
}
